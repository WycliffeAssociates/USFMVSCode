 ...
 *
 * Chapters are detected from `\c <n>` lines and verses from `\v <n>` lines.
 * A verse is only attached to the chapter that precedes it; verses that appear
 * before any chapter marker are ignored. Each symbol's `range` is closed off
 * when the next sibling begins, or at the end of the document for the last one.
 *
